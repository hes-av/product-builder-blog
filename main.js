...
// Webcam Logic
if (webcamBtn && webcamContainer && webcamWrapper && captureBtn) {
...
function resetResults() {
    if (dogProgress) dogProgress.style.width = '0%';
    if (catProgress) catProgress.style.width = '0%';
    if (dogPercent) dogPercent.textContent = '0%';
    if (catPercent) catPercent.textContent = '0%';
    if (finalVerdict) finalVerdict.textContent = '분석 중...';
    if (funFact) funFact.textContent = '';
}
...
async function handleImage(file) {
    ...
    reader.onload = async (e) => {
        ...
        if (imagePreview) predict(imagePreview);
    };
